...
export default function EditItem({match}) {
	...
	useEffect(() => {
		(
		  async () => {
			const url = `http://localhost:8007/api/lists/${match.params.id}/items/${match.params.item_id}` 
		...
		<div className="list-card-main">
			...
			<div className="row-form">
				<div className="search-field id">
					<ColumnCreateList column={"Id"} ref_current={idInput} blocked={idCheckboxBlocked} defaultValue={itemObject.id} />
				</div>
			</div>
			<div className="row-form">
				
				<div className="search-field title" >
					<ColumnCreateList column={"Title"} ref_current={titleInput} defaultValue={itemObject.title}/>
				</div>
				...
				<div className="form-group">
					<span>Owner</span>
						<select className="form-field ownerSelect" ref={ownerInput} disabled={ownerCheckboxBlocked} onChange={e => setListOwner(e.target.value)}> 
							<option disabled selected value>{itemObject.owner}</option>
								{
									Owners.map(Owner => (
										<option value={Owner}>
											{Owner}
			...
			<div className="row-form">
				<div className="search-field description">
					<ColumnCreateList column={"Description"} ref_current={descriptionInput} defaultValue={itemObject.description }/>
				</div>
			</div>
			<div className="row-form">
				<Link to="/lists">
